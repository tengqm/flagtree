...
def profile(fn, max_retry=10):
    ...
    while t == -1 and retry < max_retry:
        t = run(fn)
        if t == -1:
            print(f"### retry {retry} times to re-run test to get kernel cuda time during run {fn.__name__}")
            retry += 1
    assert t != -1, f"torch.profiler cannot grep kernel cuda time after try {max_retry} times during run {fn.__name__}"
    return t
...
def do_bench(fn, warmup=25, rep=100, grad_to_none=None, quantiles=None, fast_flush=True, return_mode="mean"):
    ...
    n_repeat = max(1, int(rep / estimate_ms))
    use_profile = os.getenv("TRITON_USE_PROFILER_DO_BENCH", False)
    if use_profile:
        n_warmup = min(50, n_warmup)
        n_repeat = min(30, n_repeat)
    else:
